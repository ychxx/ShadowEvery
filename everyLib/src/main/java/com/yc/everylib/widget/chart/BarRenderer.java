        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if (dataSet.getGradientColor() != null) {
                ...
                mRenderPaint.setShader(
                        new LinearGradient(buffer.buffer[j], buffer.buffer[j + 3], buffer.buffer[j], buffer.buffer[j + 1],
            ...
            float d = Math.abs(buffer.buffer[j] - buffer.buffer[j + 2]);
            float[] radii = {d, d, d, d, 0f, 0f, 0f, 0f};
            path.addRoundRect(rectF, radii, Path.Direction.CW);
            c.drawPath(path, mRenderPaint);
//            c.drawRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
//                    buffer.buffer[j + 3], mRenderPaint);
//            c.drawRoundRect(rectF,25,25,mRenderPaint);
            if (drawBorder) {
//                c.drawRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
//                        buffer.buffer[j + 3], mBarBorderPaint);
//                c.drawRoundRect(rectF, 25, 25, mBarBorderPaint);
            }
        }
    }
