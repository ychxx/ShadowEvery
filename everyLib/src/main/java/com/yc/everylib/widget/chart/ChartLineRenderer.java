...
public class ChartLineRenderer extends LineRadarRenderer {
    ...
    protected void drawHorizontalBezier(ILineDataSet dataSet) {
        List<LineDataSet> solidLineDataSets = new ArrayList<>();
        List<LineDataSet> dottedLineDataSets = new ArrayList<>();
        List<Entry> solidValues = new ArrayList<>();
        List<Entry> dottedValues = new ArrayList<>();
        for (int i = 0; i < dataSet.getEntryCount(); i++) {
            if (isNotEmptyEntry(i, dataSet)) {
                if (dottedValues.size() > 0) {
                ...
                if (solidValues.size() > 0) {
                    dottedValues.add(solidValues.get(solidValues.size() - 1));
                    solidLineDataSets.add(new LineDataSet(solidValues, ""));
                    solidValues = new ArrayList<>();
                }
            }
        }
        if (dottedValues.size() > 0) {
        ...
        drawHorizontalBezierMore(dataSet, solidLineDataSets, dottedLineDataSets);
    }

    private void drawHorizontalBezierMore(ILineDataSet dataSet, List<LineDataSet> solidLineDataSets, List<LineDataSet> dottedLineDataSets) {
        Transformer trans = mChart.getTransformer(dataSet.getAxisDependency());
        mXBounds.set(mChart, dataSet);
        Path solidLinePath = new Path();
        if (solidLineDataSets != null) {
            for (int i = 0; i < solidLineDataSets.size(); i++) {
                solidLinePath.addPath(createPath(solidLineDataSets.get(i)));
            }
        }
        Path dottedLinePath = new Path();
        if (dottedLineDataSets != null) {
            for (int i = 0; i < dottedLineDataSets.size(); i++) {
                dottedLinePath.addPath(createPath(dottedLineDataSets.get(i)));
            }
        }
        // if filled is enabled, close the path
        if (dataSet.isDrawFilledEnabled()) {
            cubicFillPath.reset();
            cubicFillPath.addPath(createFillPath(dataSet));
...
//        float[] positions = new float[]{0.3f, 0.3f, 0.4f};//比例
        ...
        cubicPath.reset();
        cubicPath.addPath(solidLinePath);
        trans.pathValueToPixel(cubicPath);
        mRenderPaint.setPathEffect(null);
        mBitmapCanvas.drawPath(cubicPath, mRenderPaint);

        cubicPath.reset();
        cubicPath.addPath(dottedLinePath);
        mRenderPaint.setPathEffect(new DashPathEffect(new float[]{6, 4}, 0));
        trans.pathValueToPixel(cubicPath);
        mBitmapCanvas.drawPath(cubicPath, mRenderPaint);
    ...
    protected Path createFillPath(ILineDataSet dataSet) {
        Path path = new Path();
        if (dataSet.getEntryCount() <= 1) {
            return path;
        }
        ...
        for (int i = 0; i < dataSet.getEntryCount(); i++) {
            if (isNotEmptyEntry(i, dataSet)) {
                data.add(dataSet.getEntryForIndex(i));
            }
        }
        if (data.size() >= 1) {
            Entry prev = data.get(0);
            Entry cur = prev;
            path.moveTo(cur.getX(), cur.getY() * phaseY);
    ...
    protected Path createPath(ILineDataSet dataSet) {
        Path path = new Path();
        if (dataSet.getEntryCount() <= 1) {
            return path;
        }
        float phaseY = mAnimator.getPhaseY();
        if (dataSet.getEntryCount() >= 1) {
            Entry prev = dataSet.getEntryForIndex(0);
            Entry cur = prev;
            path.moveTo(cur.getX(), cur.getY() * phaseY);
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if (dataSet.isDrawValuesEnabled() && isNotEmptyEntry(j / 2 + mXBounds.min, dataSet)) {//点上方文字
